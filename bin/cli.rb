...
def log_in
    ...
    if User.find_by(user_name: acc_name)
        ...
        if entered_password == correct
            ...
            puts "The password is #{Rainbow("incorrect").red}."
...
def play_quiz(quizapi)
    ...
    questions.each_with_index do |question, i|
        ...
        if current == correct_answers[i]
            puts "#{Rainbow("CORRECT!").green} You have earned 1 point."
            points += 1
            puts "You have #{points} points!"
        else
            puts "#{Rainbow("INCORRECT!").red} The correct answer was #{correct_answers[i]}"
        end
    end
